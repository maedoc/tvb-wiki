...
Whole-brain modeling approaches increasingly incorporate age-related changes in neural parameters to simulate brain dynamics across the lifespan. The Wong-Wang model and similar neural mass models can be parameterized to capture age-related changes in excitation-inhibition balance, which computational studies associate with altered gamma-aminobutyric acid (GABA)ergic signaling and glutamate dynamics in the aging brain [4]. The Wong-Wang model describes the dynamics of cortical populations through coupled equations for firing rates:

$$\frac{dS_i}{dt} = -\frac{S_i}{\tau_i} + (1 - S_i) \sigma\left(\sum_j G_{ij} S_j + J_NMDA S_i + I_i\right)$$

where $S_i$ represents the synaptic activity of population $i$, $\tau_i$ is the synaptic time constant, $G_{ij}$ denotes the coupling strength from region $j$ to region $i$, $J_{NMDA}$ is the NMDA-mediated self-excitation, and $I_i$ represents external input. Age-related changes in the excitation-inhibition ratio—thought to arise from altered GABAergic signaling—can be captured by modifying the effective values of $J_{NMDA}$ and the inhibitory coupling terms. Parameter estimation techniques applied to empirical neuroimaging data enable construction of personalized brain models that account for individual differences in structural connectivity derived from diffusion tensor imaging (DTI).

The concept of brain reserve—individual differences in the structural and functional capacity of the brain to cope with age-related pathology—provides a theoretical framework for understanding variability in aging outcomes that computational models aim to capture. Successful aging, characterized by maintenance of cognitive function into late adulthood despite evidence of brain pathology, represents a key target for computational investigation. Models incorporating brain maintenance mechanisms may help identify factors that promote resilient brain aging, potentially informing interventions for age-related neurological conditions.
...
1. Cheryl L. Grady. (2012). *The Cognitive Neuroscience of Ageing and Functional Reserve*. European Journal of Neuroscience. [DOI](https://doi.org/10.1111/j.1460-9568.2012.08099.x)
...
3. Stephen M. Smith, Lloyd T. Elliott, et al. (2021). *UK Biobank Brain Imaging: Structural MRI in a Massive Population Resource*. bioRxiv. [DOI](https://doi.org/10.1101/2021.05.28.446063)

4. Wang, X. J. & Wong, K. F. (2006). *Cortical signaling for perception and voluntary behavior*. Journal of Neurophysiology.