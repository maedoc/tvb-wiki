...
The motivation for such a model stems from the fundamental observation that cortical tissue contains both excitatory glutamatergic neurons and inhibitory GABAergic neurons in a tightly regulated balance. Simple [[spiking-neural-networks|integrate-and-fire]] or conductance-based models that treat neurons in isolation cannot capture this population-level interaction. The Wilson-Cowan approach instead describes the mean activity of neuronal populations, capturing the essential dynamics while remaining computationally tractable for large-scale simulations.
