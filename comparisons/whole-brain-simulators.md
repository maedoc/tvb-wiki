...
NEURON provides extensive biophysical model libraries through its MOD file mechanism, supporting detailed ion channel distributions and morphologically realistic neuronal reconstructions. While the learning curve is steeper than Python-centric platforms, the validation capabilities against experimental data remain unmatched for detail-critical applications.

Brian2 offers exceptional extensibility through its equation-based model definition system, making it particularly suitable for theoretical investigations and educational contexts where rapid iteration on model formulations is essential.
...
Sanz Leon, P., et al. (2013). The Virtual Brain: a whole-brain simulator for neuroimaging. Frontiers in Neuroinformatics.

Eppler, M., et al. (2009). PyNEST: A convenient interface to the NEST simulator. Frontiers in Neuroinformatics.

Jordan, J., et al. (2018). Massively parallel neural simulation with NEST. Proceedings of the PASC Conference.

Hines, M., & Carnevale, N. (1997). The NEURON simulation environment. Neural Computation.

Potjans, T., & Diesmann, M. (2014). The cell-type specific cortical microcircuit. Cerebral Cortex.

Fox, M., & Raichle, M. (2007). Spontaneous fluctuations in brain activity observed with functional magnetic resonance imaging. Nature Reviews Neuroscience.

Schwalger, T., Deger, M., & Gerstner, W. (2017). Towards a theory of cortical columns: From spiking neurons to neural mass models. PLoS Computational Biology.

Martin, L., et al. (2025). The Virtual Brain Ontology: A Digital Knowledge Framework for Reproducible Brain Network Modeling. bioRxiv.

Hofsähs, T., et al. (2026). The Virtual Brain links transcranial magnetic stimulation evoked potentials and inhibitory neurotransmitter changes in major depressive disorder. bioRxiv.