...
Brain-Score organizes evaluation into a tiered structure spanning multiple levels of neural and cognitive function, from activity patterns in specific brain regions to behavioral outputs such as reaction times and choice accuracy. Each benchmark pairs a set of stimuli with corresponding empirical recordings—collected under [[resting-state]] conditions, during task performance, or from passive viewing paradigms—and applies a scoring function that quantifies the similarity between model predictions and observed data. The platform aggregates performance across multiple benchmarks into a composite score, while also providing per-benchmark breakdowns that preserve transparency about which aspects of neural function a given model captures well and which it fails to reproduce.
