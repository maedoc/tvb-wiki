---
created: 2026-04-20
sources:
- raw/papers/tuckwell-1988.md
- raw/papers/freeman-1975.md
- raw/papers/ermentrout-terman-2010.md
- raw/papers/dayan-abbott-2001.md
- raw/papers/izhikevich-2007.md
- raw/papers/sporns-2011.md
- raw/papers/breakspear-2017.md
tags:
- people-researcher
title: Emanuel Tuckwell
type: entity
updated: '2026-04-30'
---

## Overview
Henry C. Tuckwell is a mathematician and theoretical neuroscientist known for his foundational work on stochastic processes in neuroscience. His two-volume work on theoretical neurobiology established mathematical frameworks for understanding neural computation.

## Key Contributions
- **Theoretical Neurobiology**: Author of the comprehensive two-volume "Introduction to Theoretical Neurobiology"
- **Stochastic Methods**: Mathematical analysis of stochastic processes in neurons
- **Cable Theory**: Stochastic extensions to cable theory for dendritic computation
- **Population Dynamics**: Mathematical models of neural population activity

## Relevance to TVB
- tuckwell-1988 provides stochastic methods for [[neural mass model]] analysis
- Population density approaches for [[whole brain]] modeling
- Foundation for [[stochastic differential equations]] in neuroscience

## Related Concepts
- [[stochastic differential equations]]
- [[neural mass model]]
- [[mean field theory]]
- [[whole brain]]

## Selected Works
- Tuckwell (1988) — Introduction to Theoretical Neurobiology, Vol 2: Nonlinear and Stochastic Theories
- Tuckwell (1988) — Introduction to Theoretical Neurobiology, Vol 1: [[linear]] Cable Theory
- Tuckwell (2005) — Spike trains in a stochastic Hodgkin-Huxley model

## References

1. (authors unknown). *Introduction to Theoretical Neurobiology: Volume 2, Nonlinear and Stochastic Theories*.
2. Walter J. Freeman. *Mass Action in the Nervous System*.
3. (authors unknown). *Mathematical Foundations of Neuroscience*.
4. Peter Dayan, Larry F. Abbott. *Theoretical Neuroscience: Computational and Mathematical Modeling of Neural Systems*.
5. Eugene M. Izhikevich. *Dynamical Systems in Neuroscience: The Geometry of Excitability and Bursting*.
6. (authors unknown). *Networks of the Brain*.
7. Michael Breakspear. *Dynamic models of large-scale brain activity*. Nature Neuroscience (Review). [DOI](https://doi.org/10.1038/s41593-017-0015-4)