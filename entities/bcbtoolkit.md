...
The **Disconnectome Map** tool represents the core innovation of BCBToolKit: rather than relying on atlas-based approximations, it employs tractography from healthy control populations to compute patient-specific maps of directly disconnected brain regions. By propagating streamlines through the lesion region, the method identifies all areas that lose their structural connection to the [[tvb-rest]] of the brain due to the lesion, producing three-dimensional probability maps that account for inter-individual variability in white matter anatomy.
