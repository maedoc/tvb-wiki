...
DPABI shares methodological foundations with several other neuroimaging toolboxes in the ecosystem. It complements [[spm]] (Statistical Parametric Mapping) and [[fsl]] (FMRIB Software Library) by providing resting-state-specific analysis routines that extend general-purpose preprocessing capabilities. For connectivity analysis, DPABI overlaps functionally with the [[brain-connectivity-toolkit]], which provides graph theoretical methods, and [[conn]], another GUI-driven connectivity analysis toolbox. The visualization components of DPABI integrate with [[afni]], [[freesurfer]], and [[connectome-workbench]], enabling multi-modal visualization workflows.
