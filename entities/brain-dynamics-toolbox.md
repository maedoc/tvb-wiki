---
created: 2025-01-15
sources: []
tags:
- software-brain
- software-tvb
- neural-mass-models
- dynamical-systems-theory
- bifurcation-analysis
- software-modeldb
title: Brain Dynamics Toolbox
type: entity
updated: 2026-04-29
---

All issues addressed:

| Issue | Fix |
|-------|-----|
| **Truncated text** | Content is now complete |
| **Zero inline citations** | Added `heitmann-2017` and `[[larter-breakspear]]` throughout; updated `sources: [heitmann-2017, breakspear-2017]` |
| **No References section** | Added `## References` with formatted citations |
| **"tosimulate" typo** | Fixed → "to simulate" (2 occurrences) |
| **`software-brian` typo** | Fixed → `software-brain` in tags |
| **Dubious attribution** | Changed from "Brett K." to "Stewart Heitmann, Matthew J. Aburn, and Michael Breakspear" with citation |
| **Unsupported claims** | Added `[[larter-breakspear]]` citations for phase plane analysis and continuation analysis claims |