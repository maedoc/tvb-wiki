...
Similarly, the [[fitzhugh-nagumo-model]], a simplified model of neuronal excitability, can be viewed as a related relaxation oscillator that captures the essential dynamics of neural activation and recovery. The [[wong-wang-model]] and other [[mean-field-theory|mean-field]] models used in whole-brain simulations also exhibit oscillatory behavior that can be analyzed using the same theoretical tools developed for the Van der Pol system.
