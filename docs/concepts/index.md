---
title: All Concepts
---

# Concepts Index

A collection of foundational concepts in computational neuroscience and whole-brain modeling, organized by domain.

## Neuroimaging

- [fMRI](](fmri.md)) – Functional magnetic resonance imaging
- [EEG](](eeg.md)) – Electroencephalography
- [XCOS](](xcos.md)) – EEG electrode coordinate system for source imaging
- [MEG](](meg.md)) – Magnetoencephalography
- [DTI](](dti.md)) – Diffusion tensor imaging
- [Diffusion MRI](](diffusion-mri.md)) – Structural connectivity from diffusion imaging
- [Tractography](](tractography.md)) – White matter fiber tracking
- [BOLD Signal](](bold-signal.md)) – Blood oxygen level-dependent contrast
- [Resting-state](](resting-state.md)) – Task-free functional connectivity

## Connectivity & Networks

- [Connectome](](connectome.md)) – Comprehensive brain wiring map
- [Connectomics](](connectomics.md)) – The field of connectome mapping
- [Functional Connectivity](](functional-connectivity.md)) – Statistical dependencies between regions
- [Structural Connectivity](](structural-connectivity.md)) – Physical white matter connections
- [Effective Connectivity](](effective-connectivity.md)) – Causal influence between regions
- [Graph Theory](](graph-theory.md)) – Mathematical tools for network analysis
- [Modularity](](modularity.md)) – Network community structure
- [Network Hubs](](network-hubs.md)) – Highly connected brain regions
- [Rich Club](](rich-club.md)) – Interconnected hub regions
- [Small-world Networks](](small-world-networks.md)) – High clustering, short paths
- [Scale-free Networks](](scale-free-networks.md)) – Power-law degree distributions

## Brain Modeling

- [Neural Mass Model](](neural-mass-model.md)) – Population-level dynamics
- [Mean-field Theory](](mean-field-theory.md)) – Macroscopic limits of networks
- [Dynamic Causal Modeling](](dynamic-causal-modeling.md)) – Inferring effective connectivity from data
- [Free Energy Principle](](free-energy-principle.md)) – Variational brain theory
- [Spiking Neural Networks](](spiking-neural-networks.md)) – Mechanistic neuron-level models
- [Stochastic Differential Equations](](stochastic-differential-equations.md)) – Noise-driven dynamics

### Neural Mass Models

| Model | Dimensions | Primary Use |
|-------|------------|-------------|
| [Jansen-Rit](](jansen-rit.md)) | 3D | EEG/MEG generation |
| [Wilson-Cowan](](wilson-cowan.md)) | 2D | General rhythms |
| [Epileptor](](epileptor.md)) | 6D | Seizure dynamics |
| [EpileptorCodim3](](epileptorcodim3.md)) | 2D | Bifurcation analysis |
| [EpileptorRS](](epileptor-rs.md)) | 6D+ | Resting state |
| [Wong-Wang](](wong-wang.md)) | 1D | fMRI/BOLD |
| [Wong-Wang E-I](](wong-wang-exc-inh.md)) | 2D | E-I balance |
| [Larter-Breakspear](](larter-breakspear.md)) | 3D | Ion channel dynamics |
| [Zerlaut](](zerlaut.md)) | 2D+adapt | Adaptation effects |
| [Stefanescu-Jirsa](](stefanescu-jirsa.md)) | 2D | Reduced seizures |
| [K-Ion Exchange](](k-ion-exchange.md)) | 3D | Metabolic coupling |
| [Infinite Theta](](infinite-theta.md)) | 2D | Exact mean-field |
| [Hopfield](](hopfield.md)) | N | Associative memory |
| [Oscillator](](oscillator.md)) | 2D | Universal dynamics |
| [Linear](](linear.md)) | 1D | Testing/baseline |

## Dynamics & Mathematics

- [Dynamical Systems Theory](](dynamical-systems-theory.md)) – Phase space, stability, bifurcations
- [Nonlinear Dynamics](](nonlinear-dynamics.md)) – Chaos, attractors, limit cycles
- [Bifurcation Analysis](](bifurcation-analysis.md)) – Qualitative changes in dynamics
- [Bifurcation Theory](](bifurcation-theory.md)) – Classification of transitions
- [Variational Bayes](](variational-bayes.md)) – Approximate Bayesian inference
- [Fokker-Planck Equation](](fokker-planck-equation.md)) – Distribution evolution under noise
- [Epilepsy Modeling](](epilepsy-modeling.md)) – Seizure dynamics and prediction

## Biological Context

- [Aging](](aging.md)) – Age-related brain changes
- [Neurodevelopment](](neurodevelopment.md)) – Brain development trajectories
- [Cognitive Reserve](](cognitive-reserve.md)) – Protective factors against decline
- [Brain Maintenance](](brain-maintenance.md)) – Preserving brain structure
- [Developmental Trajectories](](developmental-trajectories.md)) – Changes across lifespan

## Data Analysis

- [ICA](](ica.md)) – Independent component analysis
- [Structural Core](](structural-core.md)) – Dense connectivity backbone
- [Personalized Brain Modeling](](personalized-brain-modeling.md)) – Subject-specific simulations
- [Whole-brain](](whole-brain.md)) – Macroscale network simulation
- [MEG/EEG Toolbox](](meg-eeg-toolbox.md)) – Software environments for MEG/EEG analysis

---

*Total: 59 concepts*